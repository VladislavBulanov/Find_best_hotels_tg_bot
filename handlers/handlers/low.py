...
from utils.misc.next_step_registration import (
    check_validation_result_and_register_next_step,
)
from utils.misc.offers import get_offers
from utils.misc.validation_checkin_date import validate_checkin_date
from utils.misc.validation_checkout_date import validate_checkout_date
from utils.misc.validation_guests_amount import validate_guests_amount
from utils.misc.validation_results_amount import validate_results_amount
...
def request_city_and_country(message: Message) -> None:
    ...
    logger.info(f"The command '/low' is launched by {message.from_user.full_name}")
    ...
    if result:
        ...
        if isinstance(data, List):
            if data:
                ...
                logger.info("The command '/low' is completed")
            else:
                bot.send_message(
                    message.chat.id,
